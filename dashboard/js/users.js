...
function renderUsersTable(users) {
    ...
    if (!users || users.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" class="empty-row"><div class="empty-state"><span class="empty-icon">👤</span><span>Không có user nào</span></div></td></tr>';
...
function openUserModal(userId) {
    ...
    if (userId) {
        ...
        document.getElementById('form-user-phone').value = user.phoneNumber || '';
...
async function saveUser() {
    ...
    const phoneNumber = document.getElementById('form-user-phone').value.trim();
    const password = document.getElementById('form-user-password').value;
    ...
    try {
        const body = { username, email, phoneNumber, role };
        if (password) body.password = password;

        if (id) {
            await fetchAPI(`/users/${id}`, { method: 'PUT', body: JSON.stringify(body) });
            showToast('Cập nhật user thành công!', 'success');
            if (typeof logAudit === 'function') logAudit('UPDATE', 'USER', username, `Cập nhật user: role=${role}, email=${email}, phone=${phoneNumber}`);
        } else {
            await fetchAPI('/users', { method: 'POST', body: JSON.stringify(body) });
            showToast('Tạo user mới thành công!', 'success');
            if (typeof logAudit === 'function') logAudit('CREATE', 'USER', username, `Tạo user mới: role=${role}, email=${email}, phone=${phoneNumber}`);
        }
