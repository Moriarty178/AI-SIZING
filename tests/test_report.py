...
class TestGopPhamVi:
    ...
    def test_trich_dan_quy_tac_chi_con_MOT_lan(self):
        ...
        thay vì 99. Gộp theo phạm vi nói MỘT lần; thân rút ở biên từ (nguyên văn
        đầy đủ nằm ở phụ lục — test ở `test_bao_cao_gon.py`), nên số lần lặp
        trong thân không còn phụ thuộc số phân hệ."""
        from src.reporting.report import _render_list, load_labels
        van = "\n".join(_render_list([_f_pham_vi(f"ph{i}") for i in range(13)],
                                     load_labels()))
        assert van.count("TIÊU CHÍ DÀI") <= 20      # một lần rút gọn, không phải 13
        assert van.count("TIÊU CHÍ DÀI") < 13 or "…" in van
