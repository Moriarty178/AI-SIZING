...
public class DefaultUserLoader implements CommandLineRunner {
    ...
    public DefaultUserLoader(UserService userService, UserRepository userRepository) {
    ...
    private List<CreateUserRequest> loadFromEnv() {
        ...
        for (String entry : entries) {
            ...
            // Format: username:password:role[:phoneNumber]
            String[] parts = e.split(":", 4);
            if (parts.length < 2) continue;
            String username = parts[0].trim();
            String password = parts[1].trim();
            String role = parts.length >= 3 ? parts[2].trim() : "user";
            String phone = parts.length >= 4 ? parts[3].trim() : null;

            // Check if user exists - update phone number if missing
            userRepository.findByUsername(username).ifPresent(existingUser -> {
                if ((existingUser.getPhoneNumber() == null || existingUser.getPhoneNumber().isBlank())
                    ...
                    log.info("Updated phone number for existing user {}: {}", username, phone);
                }
            });
